...
const eyeL = new Path2D(EYE_L)
const eyeR = new Path2D(EYE_R)

export function paintGrokFace(
  ...
  ctx.scale((size * 0.84) / 259, (size * 0.8) / 259)
  ctx.translate(15, 15)
  ctx.translate(114.2705, 114.2705)
  ctx.scale(1.040524017467249, 1.040524017467249)
  ctx.translate(-114.2705, -114.2705)
  ctx.fillStyle = '#f4f1ea'
  ctx.fill(headPath)
  ctx.fillStyle = '#101217'
  ctx.translate(glanceX, glanceY)
  ctx.fill(eyeL)
  ctx.fill(eyeR)
  ctx.restore()
}

export function createFaceTexture(): { texture: THREE.CanvasTexture; paint: (x: number, y: number) => void } {
  const canvas = document.createElement('canvas')
  canvas.width = 512
  canvas.height = 512
  const ctx = canvas.getContext('2d')
  if (!ctx) throw new Error('Could not draw the face screen')
  paintGrokFace(ctx, 512, 0, 0)
  const texture = new THREE.CanvasTexture(canvas)
  texture.colorSpace = THREE.SRGBColorSpace
  texture.anisotropy = 8
  const paint = (x: number, y: number) => {
    paintGrokFace(ctx, 512, x, y)
